...
from model import eeg_small as eeg

# Basic model parameters as external flags.
FLAGS = None

def loadEEGData():
    dataPath = 'F:/Deep Learning/eeg/dataset/health3/'
    ...
    X_train = np.reshape(X_train, (-1, 27, 31, 1))
    y_train = np.reshape(y_train, (-1,))
    X_test = np.reshape(X_test, (-1, 27, 31, 1))
...
def run_training():
    ...
    with tf.Graph().as_default():
        images_placeholder = tf.placeholder(dtype=tf.float32, shape=[None, 27, 31, 1], name='images-input')
