...
def inference(images_placeholder, is_training,
    ...
    with tf.name_scope('dense1'):
        pool_flat = tf.reshape(pool, [-1, 18 * 13 * depth3])
    ...
    # dense2 125 output units
    with tf.name_scope('dense2'):
        logits = tf.layers.dense(inputs=dropout, units=125)
        tf.summary.histogram('dense2', dense)

    return logits

def inference_large(images_placeholder, is_training,
              depth1, depth2, depth3, dense1_units, dense2_units,
              dropout_rate=0.5):
    """
    Build the eeg model
    ...
    with tf.name_scope('conv1'):
        ...
        bn2 = tf.layers.batch_normalization(inputs=conv, training=training_mode)
        tf.summary.histogram('batch norm2', bn2)
        ...
        tf.summary.histogram('conv layer2:', conv2)
