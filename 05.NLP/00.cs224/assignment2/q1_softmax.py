...
def cross_entropy_loss(y, yhat):
    ...
    Args:
        ...
        yhat(softmax result): tf.Tensor with shape (n_sample, n_classes). Each row encodes a
    ...
    result = tf.to_float(y) * yhat
    yhatMax = tf.reduce_max(result, axis=1)
    out = - tf.reduce_sum(tf.log(yhatMax))
